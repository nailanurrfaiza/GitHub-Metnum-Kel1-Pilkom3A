...
void gaussJordan(double arr[size][size], int ordo){
    ...
    // perulangan semua matriks
    for(i = 0; i < ordo; i++){
        pembagi = arr[i][i];

        // perulangan tiap baris pada matriks
        for(k = i; k < ordo + 1; k++){
            arr[i][k] /= pembagi;
        }

        // perulangan tiap kolom pada matriks dan cek secara vertikal agar array selain x[i][i] bernilai 0
        for(j = i + 1; j < ordo + 1; j++){
            v[i] = arr[j][i];

            // perulangan tiap baris pada matriks kolom yang sama
            for(k = i; k < ordo + 1; k++){
...
int main(){
    ...
    for(i = 0; i < ordo; i++) {
        ...
        for(j = 0; j < ordo; j++){
            ...
            // vektor B disimpan ke dalam array dimensi kedua indeks terakhir jika telah sampai pada ujung baris (ordo - 1), input vektor B ke array arr[i][j+1]
            if(j == ordo - 1){
                printf("Matriks B\n");
                printf("[%d][%d] = ", (i+1), (j+1));
                scanf("%lf", &arr[i][j + 1]);
